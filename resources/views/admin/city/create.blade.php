...
@section('content')
    ...
    <form method="post" action="{{ route('admin.city.store') }}" accept-charset="utf-8">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select data-placeholder="选择城市..." class="chosen-select" style="min-width:200px;" name="pid">
                                    ...
                                    <option value="0" {{ (Input::old('pid')  === 0) ? 'selected' : '' }}>顶级城市</option>

                                @foreach ($cities as $li)
                                    <option value="{{ $li->id }}" {{ (Input::old('pid')  === $li->id) ? 'selected' : '' }}>{{ $li->name }}</option>
