...
@section('title'){{ $marketAc -> name }}  数据统计 会员卡管理系统 @stop
{{-- 高亮显示 --}}
...
@section('content-header')
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">{{ $marketAc -> name }} 数据统计</li>
    </ol>
@stop

@section('content')
    <a href="{{ route('card.data.index') }}" class="btn bg-navy margin-bottom">全部市场</a>
    @foreach($markets as $li)
        @if($li->id == $marketAc -> id)
            <a href="#" class="btn bg-maroon margin-bottom">{{ $li->name }}</a>
        @else
            <a href="{{ route('card.data.index') }}/{{ $li->id }}" class="btn bg-purple margin-bottom">{{ $li->name }}</a>
        @endif
    @endforeach
    <div class="row">
        <div class="col-xs-12">
